    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult CreateRole(CreateRoleViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (!rm.RoleExists(model.RoleName))
                {
                    rm.Create(new IdentityRole(model.RoleName));
        ...
        public ActionResult EditUser(string id, ManageMessageId? Message = null)
        {
            ...
            model.AllRoles = new SelectList(Db.Roles.ToList(), "Id", "Name");
            model.AllRoles = Db.Roles.Select(r => new SelectListItem { Value = r.Id, Text = r.Name }).ToList();
            foreach(var item in model.AllRoles)
            {
                if (user.Roles.FirstOrDefault(r => r.RoleId == item.Value) != null)
                {
                    item.Selected = true;
                }
            }
        ...
        public ActionResult EditUser(EditUserViewModel model)
        {
            if (ModelState.IsValid)
            {
                // save the user accounts DB changes
                var Db = new ApplicationDbContext();
