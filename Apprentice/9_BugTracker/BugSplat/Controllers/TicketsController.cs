...
namespace BugSleuth.Controllers
{
    public class TicketsController : Controller
    {
        ...
        // static project id maintains project ticket list vs. complete list
        ...
        // static model provides compound sorting
        ...
        public ActionResult Create([Bind(Include = "Id,Title,Description,Created,Updated,ProjectId,TicketTypeId,TicketPriorityId,TicketStatusId,OwnerUserName,AssignedToUserName")] Ticket ticket)
        {
            if (ModelState.IsValid)
            {
                ...
                //ticket.OwnerUserName = User.Identity.Name;
        ...
        public ActionResult Edit([Bind(Include = "Id,Title,Description,Created,Updated,ProjectId,TicketTypeId,TicketPriorityId,TicketStatusId,OwnerUserName,AssignedToUserName")] Ticket ticket, int? projectid)
        {
            if (ModelState.IsValid)
            {
                // retrieve the old version of the ticket
                var old_ticket = (Ticket)TempData["ticket"];
                ...
                {
                    db.TicketHistories.Add(new TicketHistory { 
                        Property = "Priority", Changed = DateTimeOffset.Now, 
                        UserName = User.Identity.Name, 
                        TicketId = ticket.Id, 
                ...
                {
                    db.TicketHistories.Add(new TicketHistory { 
                        Property = "Status", Changed = DateTimeOffset.Now, 
                        UserName = User.Identity.Name, 
                        TicketId = ticket.Id, 
                        OldValue = old_ticket.TicketStatus.Name, 
                        NewValue = db.TicketStatuses.Find(ticket.TicketStatusId).Name });
                } 
                if (old_ticket.AssignedToUserName != ticket.AssignedToUserName)
                {
                    db.TicketHistories.Add(new TicketHistory { 
                        Property = "Assigned", Changed = DateTimeOffset.Now, 
                        UserName = User.Identity.Name, 
                        TicketId = ticket.Id, 
                        OldValue = old_ticket.AssignedToUserName, 
